...
// These tests deliberately never invoke newServeCmd()'s RunE: doing so
// would start server.Run, a long-running process, which is exactly what
// this project's own incident (a previous agent binding a fixed port,
// finding it occupied, and killing the unrelated process holding it) says
// never to do in a test, a smoke check, or manual verification. Every test
// below exercises resolveServeDBPath, the flag definitions, and the two
...
// bounded-review finding that a zero-flag `aliasdeck serve` bound every
// interface: defaultServeAddr must name an explicit loopback host, never
// an empty host (which net.Listen("tcp", …) treats as the wildcard
// address).
...
// TestNewServeCmdAddrFlagDefaultsToLoopbackAndDocumentsWidening asserts the
...
// default and what widening it means, per the finding's explicit ask.
func TestNewServeCmdAddrFlagDefaultsToLoopbackAndDocumentsWidening(t *testing.T) {
	cmd := newServeCmd()

	flag := cmd.Flags().Lookup("addr")
	if flag == nil {
		t.Fatal("newServeCmd() has no --addr flag")
	}
...
// TestNewServeCmdDBFlagDefaultsToEmpty asserts --db's default is empty
// (meaning "derive it from config.Base"), matching resolveServeDBPath's own
// precedence.
func TestNewServeCmdDBFlagDefaultsToEmpty(t *testing.T) {
	cmd := newServeCmd()

	flag := cmd.Flags().Lookup("db")
	if flag == nil {
		t.Fatal("newServeCmd() has no --db flag")
	}
	if flag.DefValue != "" {
...
// t.TempDir() via ALIASDECK_HOME exactly like main_test.go's other
...
// base directory being pre-existing (e.g. `init` already ran) is not an
// error: MkdirAll on an existing directory is a documented no-op.
...
// project does not depend on — and is reported as such rather than
...
// decision's terminal branch: server.Config.BootstrapPasswordFile must
// stay empty so auth.Bootstrap keeps printing directly to a real console,
