...
test("renders local text and updates the visible time zone label", () => {
  ...
  const timestamp = {
    ...
    dataset: { utc: "2030-01-01 00:00 UTC" },
  ...
  const label = { textContent: "Timestamps are shown in UTC." };
  const document = {
  ...
  assert.match(timestamp.title, /^Local time \(.+\)\. UTC: 2030-01-01 00:00 UTC$/);
  assert.equal(attributes.get("aria-label"), timestamp.title);
  assert.match(label.textContent, /^Timestamps are shown in your local browser time \(.+\)\.$/);
});
