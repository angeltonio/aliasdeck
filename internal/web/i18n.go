...
var messages = map[language]map[string]string{
	languageEnglish: {
		...
		"nav.profiles":                 "Groups",
		"devices.rotate":               "Rotate token",
		"devices.rotate_confirm":       "Rotate the credential for %q? Its current token stops working immediately. You will get a command to run on that machine, shown once.",
		"devices.rotated_title":        "New credential for %s",
		"devices.rotated_hint":         "Run this on that machine. It is shown once and cannot be retrieved again — rotate once more if you lose it.",
		"error.device_rotate":          "could not rotate that device's credential",
		"error.device_rotate_orphaned": "the old token was revoked but a replacement could not be issued, so that device has no working credential until you rotate again — retrying is safe",
		"devices.revoked":              "revoked", "devices.revoke": "Revoke access",
		"devices.revoke_confirm":    "Revoke %q? Its token stops working immediately and the machine cannot sync again until it is enrolled anew. Aliases and group membership are kept.",
		"error.device_revoke":       "could not revoke that device",
		"error.device_revoke_token": "the device was marked revoked but its token could not be revoked, so it may still sync — try again",
	...
	languageSpanish: {
		...
		"nav.profiles":                 "Grupos",
		"devices.rotate":               "Rotar token",
		"devices.rotate_confirm":       "¿Rotar la credencial de %q? Su token actual deja de funcionar de inmediato. Vas a recibir un comando para ejecutar en esa máquina, que se muestra una sola vez.",
		"devices.rotated_title":        "Credencial nueva para %s",
		"devices.rotated_hint":         "Ejecutá esto en esa máquina. Se muestra una sola vez y no se puede recuperar; si la perdés, rotá otra vez.",
		"error.device_rotate":          "no se pudo rotar la credencial de ese dispositivo",
		"error.device_rotate_orphaned": "el token anterior fue revocado pero no se pudo emitir el reemplazo, así que ese dispositivo no tiene credencial válida hasta que rotes de nuevo; reintentar es seguro",
		"devices.revoked":              "revocado", "devices.revoke": "Revocar acceso",
		"devices.revoke_confirm":    "¿Revocar %q? Su token deja de funcionar de inmediato y la máquina no podrá volver a sincronizar hasta que la enroles otra vez. Los alias y la pertenencia a grupos se conservan.",
		"error.device_revoke":       "no se pudo revocar ese dispositivo",
		"error.device_revoke_token": "el dispositivo quedó marcado como revocado pero no se pudo revocar su token, así que todavía podría sincronizar; probá de nuevo",
