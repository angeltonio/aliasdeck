...
- [x] 4.5 Created `cmd/aliasdeck/serve.go` — flags (`--addr`, `--db`) and `os.Getenv`/`cmd.OutOrStdout()` resolve to a `server.Config`; the default database path comes from `config.Base` + `server.db` (no change to `internal/config` itself). It is the only file in `main` importing `internal/server`; already registered in `cmd/aliasdeck/root.go` from the Phase 1 stub (`newServeCmd()` was already in `AddCommand`), so no further root.go edit was needed.
- [x] 4.6 Verify: `internal/archtest/deps_test.go` (1.5) and `go vet ./...` both green with real server wiring present (`internal/server` now imports `internal/auth`, `internal/store`, `internal/store/sqlitestore` — none of it imports `internal/renderers`). `go test -count=1 ./...` green; manual smoke of `./aliasdeck serve` confirmed a one-time bootstrap password, `GET /api/v1/health` returning `200 {"status":"ok"}`, and a clean `SIGTERM` exit (code 0). See apply-progress for the four required mutation results (all confirmed) and the six cross-compile sizes against the 25 MB budget.
...
- [ ] 5.14 **Bounded-review addition**: bound concurrent password verification (design's Bounded Operations table, "Concurrent password verification"). Measured cost: ~12.8 ms wall / 64 MiB resident per `auth.VerifyPassword`/`HashPassword` call, so 10 concurrent `POST /login` attempts alone already hold ~640 MiB — an unbounded-parallelism lever on an unauthenticated route once Phase 4/5 expose it. Add a small fixed-size semaphore (e.g. a buffered channel sized in the low single digits) that `internal/api/auth.go`'s login handler acquires immediately before calling `auth.VerifyPassword` and releases immediately after; excess concurrent attempts queue behind the semaphore, bounded in turn by the handler's existing `http.TimeoutHandler` 20s bound (5.2) — never a second, separate timeout. Land alongside 5.9/5.10 (`internal/api/auth.go`), not before. Whether the semaphore lives in `internal/auth` (bundled with the cost it bounds) or `internal/api` (bundled with the route it protects) is this task's own open call — record the choice and rationale in `design.md` when it is made, rather than deciding it unilaterally in code.
