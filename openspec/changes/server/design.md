...
| 5 | Migrations | **Confirmed as proposed (Phase 1 open item 1.2).** `pressly/goose/v3` in library mode over `embed.FS` at `internal/store/migrations/NNNN_*.sql`, forward-only, each file in one transaction. Startup pre-flight compares `goose.GetDBVersion` against the highest embedded version and **refuses to start when the database is newer**. | golang-migrate (source drivers pull networked backends); a hand-rolled runner; `migrations/` at the repo root | Go `embed` cannot escape its package directory, so root `migrations/` is not implementable — §10 is corrected here the same way it is corrected for `cmd/aliasdeck-server/`. The newer-database refusal is ours regardless of runner — it is what makes a binary downgrade a loud failure instead of silent corruption — so this choice is ergonomics (library-mode Go API vs. golang-migrate's source-driver model), not correctness. |
| 6 | Queries | `sqlc` generates `internal/store/sqlitestore` from `query.sql`; generated code is checked in and CI fails on `sqlc generate` diff. `sqlc` itself is invoked via a version-pinned `go run github.com/sqlc-dev/sqlc/cmd/sqlc@<version>` in the Makefile/CI, **not** tracked as a `go.mod` `tool` directive. | An ORM; hand-written `database/sql`; a `go.mod` `tool` entry for `sqlc` | §9.3. The drift check is what keeps the checked-in code honest. The `tool` directive was tried during Phase 1 (task 1.3) and reverted: `sqlc`'s own dependency tree (Postgres/MySQL parsers, wazero, gRPC) is unrelated to what AliasDeck ships, and pulling it into the main module's `go.mod` forced an automatic `go` directive bump from `1.25.7` to `1.26.0` — silently drifting past the asdf-pinned `1.25.11` toolchain. `go run ...@version` gets the same reproducibility without widening `go.sum` or the module's minimum Go version. |
...
| 17 | `login`/`logout` semantics | **Confirmed as proposed (Phase 1 open item 1.1).** `login` authenticates the *operator* only: it exchanges a username/password for a `session` token (decision 8) and writes it to the credentials file (decision 14) — never to `config.yaml`. `logout` clears that local session file only; it never contacts the server, so it always succeeds even when the server is unreachable. Revoking the session server-side (invalidating it before its 24 h expiry) is a deliberate non-goal of `logout` in v0.3. | Making `logout` call `POST /api/v1/auth/logout` to revoke the session server-side before clearing the local file | A local-only `logout` cannot fail on a network partition — exactly the "hard error naming the URL, no cache" posture decision 11 already applies to sync, but here inverted: removing a credential must not depend on reachability. Session tokens already expire at 24 h (decision 8's lifetime table) and `RevokeSubject` gives an operator a server-side "log out everywhere" if a token is suspected leaked — that is a distinct, deliberate operator action, not something `logout` performs implicitly. `register`'s device token is unaffected either way: it is a separate credential with its own separate revocation. |
| 18 | Foreign-key violation sentinel | `store.ErrInvalidReference`, distinct from `ErrConflict` and `ErrNotFound`. `mapWriteError` (`sqlitestore/convert.go`) checks `SQLITE_CONSTRAINT_FOREIGNKEY` (787) before masking to the primary code, so a dangling reference (e.g. `Alias.ProfileIDs` naming a profile that does not exist) maps here instead of collapsing into `ErrConflict` alongside `SQLITE_CONSTRAINT_UNIQUE` (2067). | Reusing `ErrConflict` (masking with `&0xff` alone maps every extended `SQLITE_CONSTRAINT_*` code to 19, so a dangling reference was indistinguishable from a name collision — `errors.Is(err, ErrConflict)` was literally true for both); reusing `ErrNotFound` (conflates "the record you asked for by id is missing" with "a value inside your payload references a nonexistent record") | Bounded-review finding (post-Phase-2 correction pass): creating an alias whose `ProfileIDs` names a nonexistent profile returned `store: conflict`, which Phase 4 would have mapped to HTTP 409 — a dangling reference is a 400/422, not a name collision. A third sentinel keeps that HTTP mapping decision (deferred to Phase 5) open rather than forcing it prematurely onto whichever of the other two happened to be closest. |
| 19 | Database file permissions | `sqlitestore.Open` creates the database file at mode `0600` before ever handing the path to the driver, and tightens it if the file already existed; the WAL/SHM sidecars get the same mode once they exist. | Leaving the file at the process umask's default | Bounded-review finding: the database holds operator password hashes and token secret hashes — the same class of secret decision 14's `credentials.json` is `0600` for — and had no equivalent protection. Same Windows gap and compensating controls as decision 14's "Windows 0600 Gap" note below; not a second convention. |
| 20 | Operator username | v0.3 bootstraps exactly one operator account, at the fixed username `admin` (`internal/auth/bootstrap.go`'s unexported `bootstrapUsername`); `login` always authenticates against that username. | A `--username` bootstrap flag; prompting for a username | Bounded-review finding: the fixed username was previously discoverable only by reading an unexported constant — neither this table nor `server-auth/spec.md`'s "One Operator Account" requirement named it, despite Phase 8's `login` depending on it. "One operator account" (spec requirement) is what makes the username not a secret and not worth making configurable before that scope changes; recording it here is documentation of existing behavior, not a behavior change. |
| 21 | Default bind address | `aliasdeck serve --addr` defaults to `127.0.0.1:8080` (loopback only). Widening the bind (e.g. `0.0.0.0:8080`, or a specific interface behind a reverse proxy) remains fully supported, but must now be a deliberate `--addr` value the operator names; the flag's own help text states both the default and what widening it means. | Keeping the previous default, an empty host (`":8080"`), which `net.Listen("tcp", …)` binds as the wildcard address | Bounded-review finding (post-Phase-4 correction pass, the same pass that produced a real incident: an agent on this branch picked a fixed port for a manual smoke test, found it occupied, and killed the unrelated process holding it — `com.docker.backend`, taking four running containers down with it). A zero-flag `aliasdeck serve` was reachable from the LAN behind nothing but a bootstrap password printed once. It also directly contradicted this project's own client-side policy: decision 13 makes `ServerSource` refuse any non-HTTPS URL unless the host is loopback, re-checked on every sync — so the default server configuration was producing exactly what the default client refuses to talk to. That incoherence is the strongest argument the previous default was simply wrong, not merely conservative. |
| 22 | Bootstrap password delivery when stdout is not a terminal | `aliasdeck serve` checks whether its stdout is a real console, using `github.com/mattn/go-isatty` (promoted from an indirect to a direct dependency) against the underlying file descriptor — only possible when the writer is an `*os.File`, which is the caller's own check (`cmd/aliasdeck/serve.go`'s `isTerminalWriter`), never `internal/auth`'s. When stdout is a terminal, `auth.Bootstrap` prints the generated password to it exactly as before. When it is not, `auth.Bootstrap` instead writes the password to a `0600` file atomically (the same temp-file-then-rename pattern `internal/state.Save` already uses for `state.json` — this project's second use of that pattern, not a third convention), placed as a sibling of the database file, and prints only that file's path to stdout. | Printing unconditionally to stdout; redirecting to stderr instead | Bounded-review finding: `server-auth/spec.md` states, twice, that the generated password "MUST NOT ... be written to any log" and is "printed once to the console, and never written to a log." `server-runtime/spec.md`'s "Zero Stdin Prompts" scenario requires `serve` to work "started with stdin closed, as under systemd" — and systemd's default `StandardOutput=journal` (and `StandardError`, the same destination) makes stdout *the journal itself*: persistent, `journalctl`-queryable, and readable by more than the invoking user on a typical distribution. The code honored the letter of "never write to a logger" (it never called one) while breaking the promise on exactly the deployment the spec requires supporting. A `0600` file is this project's existing answer for a live secret (`credentials.json`, decision 14; the database file, decision 19); inventing a third mechanism was rejected in favor of that convention. Delivery happens before the operator row is created, so a failure to deliver (e.g. the file cannot be written) leaves no operator created — a retry on the next start can attempt again cleanly instead of leaving an operator whose password nobody ever received. |
| 23 | Health route ownership across phases | `GET /api/v1/health` is wired directly by `internal/server/handler.go` ahead of Phase 5's full router, and is unauthenticated by design (server-runtime spec, "Health Endpoint Requires No Authentication"). Phase 5's route slice (decision 15) MUST include it and MUST NOT re-guard it behind any token-kind check. | Leaving this constraint only in `handler.go`'s code comments, undiscoverable from this table the way decisions 9 and 15 (the other route-shape decisions) are | Bounded-review finding: comparable route decisions already live here (9: sync's shape; 15: the route-slice/OpenAPI coverage mechanism), but the health path's own "must stay reachable without auth, forever" constraint lived only in a doc comment Phase 5 has no structural reason to read before building its route slice. Recording it here gives Phase 5's own RED test (5.1, "a route missing its required-token-kind declaration fails registration") a named requirement to check `/api/v1/health` against, rather than relying on nobody deleting a comment. |
| 24 | Login concurrency semaphore ownership (task 5.14's open call) | The fixed-size semaphore bounding concurrent `auth.VerifyPassword`/`HashPassword` calls lives in `internal/api` — specifically as an unexported, package-level `chan struct{}` in `internal/api/auth.go`, acquired and released only around the `login` handler's call to `auth.VerifyPassword` — not in `internal/auth`. | Exporting the bound from `internal/auth` (e.g. a package-level semaphore `VerifyPassword` itself acquires, or a wrapper `auth.VerifyPasswordBounded`) so every caller of the KDF shares one limiter | The resource being bounded is concurrent *reachability* of an unauthenticated HTTP route, not the cost of the argon2id algorithm itself — the same category of concern `http.TimeoutHandler` (decision, Bounded Operations "Handler execution") already bounds in `internal/api/router.go`, and the bounded-operations table explicitly ties the semaphore's overflow to that same 20s timeout ("bounded in turn by the handler's existing `http.TimeoutHandler` 20s bound ... never a second, separate timeout") — a relationship that stays legible only if both live in the same package. `internal/auth`'s own package doc describes it as pure identity/crypto primitives (minting, hashing, kind checks, bootstrap) with zero HTTP awareness; it has no request, no route, and no concept of "this call arrived over an unauthenticated network boundary". Putting the limiter there would also silently throttle `auth.Bootstrap`'s own one-time `HashPassword`/`VerifyPassword` calls at startup — a single-threaded, already-serialized call with a completely different risk profile — against the same pool sized for an adversarial `POST /login`, coupling two callers that share nothing but the function they happen to call. This mirrors decisions 9 and 23: a route's own exposure-driven behavior stays next to the route, not pushed into a lower-level package it merely calls. |
...
| Accept / read / write | `http.Server{ReadHeaderTimeout: 5s, ReadTimeout: 15s, WriteTimeout: 30s, IdleTimeout: 60s, MaxHeaderBytes: 64<<10}`. **Rationale (bounded-review finding, suggestion 10)**: none of these five values is a measurement against this project's own traffic — there is none yet to measure. They are conventional defaults for a single-operator personal control plane: `ReadHeaderTimeout`/`ReadTimeout` track values commonly recommended for small internal APIs; `WriteTimeout` is double `ReadTimeout` as headroom above the store's own 5s `busy_timeout` (decision 7) and the 20s `http.TimeoutHandler` (Phase 5) that already bound the slowest handler; `IdleTimeout` is a conventional keep-alive window nothing here depends on; `MaxHeaderBytes` matches Go's own `http.DefaultMaxHeaderBytes`, restated explicitly so a future change is a visible diff. House style (`GitTimeout`, `internal/source/gitrun.go`) is that a magic number carries the failure or measurement that produced it; stating plainly that these five were adopted without one is the honest alternative to inventing a false provenance. | `internal/server/server.go` |
...
| Operator bootstrap | Password from `crypto/rand` or `ALIASDECK_ADMIN_PASSWORD`, printed once to a terminal stdout, or written once to a `0600` file when stdout is not a terminal (decision 22). **Zero stdin reads in `serve`** | `internal/auth/bootstrap.go` (delivery), `cmd/aliasdeck/serve.go` (terminal detection) |
...
| Concurrent password verification | **Not yet bound — bounded-review finding.** `VerifyPassword`/`HashPassword` measured at ~12.8 ms wall time and 64 MiB resident per call on this project's reference hardware, so 10 concurrent `POST /login` attempts alone already hold ~640 MiB; no task in Phases 4–10 caps concurrent login attempts, which is the same "unbounded operation" failure family this project has now hit three times (see `internal/source/gitrun.go`'s `GitTimeout` comment), manifesting here as unbounded *parallelism* rather than an unbounded *wait*. Proposed fix, to land with Phase 5 (tasks.md 5.14): a small fixed-size semaphore that `internal/api/auth.go`'s login handler acquires around `auth.VerifyPassword` and releases after, so excess concurrent attempts queue behind the handler's existing `http.TimeoutHandler` 20s bound (row above) instead of each allocating a full argon2id block in parallel. **Resolved (decision 24)**: the semaphore lives in `internal/api/auth.go`, not `internal/auth` — implementation remains for the second pass alongside 5.9/5.10. | `internal/api/auth.go` (decision 24; proposed, not yet implemented) — Phase 5 |
...
The v0.2 gap now covers a file holding a live server credential, and — as of decision 19 — the server's own database file. Go's `Chmod` on Windows toggles only the read-only bit; `0600` is not an ACL. Mitigation is layered, and none of the layers is "we fixed permissions". This is one gap and one set of mitigations, deliberately not duplicated per file:

1. **Blast radius, by construction.** `credentials.json` holds a *device* token and nothing else — that is what the enrollment-token decision buys. A stolen device token reads one device's aliases; it cannot create aliases, devices, profiles or tokens, and it is not an operator session. The database file is the control plane's full state, so this layer buys less for it than for `credentials.json` — layers 2–4 below carry correspondingly more weight there.
2. **Placement.** The file sits under `%USERPROFILE%\.config\aliasdeck`, whose inherited ACL is already user + administrators. The gap is "not enforced by us", not "world-readable by default". The database file (decision 19) sits alongside it, under the same inherited ACL.
...
| **Database file permissions** | Applicable | `sqlitestore.Open` creates/tightens the `.db` file (and its WAL/SHM sidecars) to `0600` before the driver ever touches it (decision 19); same Windows gap and compensating controls as the credential file | A database file created through `Open` must be `0600` on a POSIX filesystem, including when re-opening a pre-existing file created at a wider mode |
...
| **Default network exposure** | Applicable | `--addr` defaults to loopback only (decision 21); widening it to any other interface requires an explicit, documented operator choice | A default `aliasdeck serve` invocation with no flags must bind loopback only, never the wildcard address |
| **Bootstrap password destination** | Applicable | Terminal detection at the caller picks console printing vs. a `0600` file (decision 22); the password is never handed to a logger under either path | A non-terminal stdout must not receive the generated password itself; it must be written to a `0600` file instead, with only that path appearing on stdout |
