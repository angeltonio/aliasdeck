...
| 8 | `$PROFILE` resolution | New `internal/app/pwshprofile.go`: `resolvePowerShellProfile(env, platform) (pwshProfile, error)` returning `{Path, Edition, Provenance, OtherPath, OtherExists}`. Precedence: `--rc-file` → `$ALIASDECK_PWSH_PROFILE` (test seam) → `LookPath("pwsh")` ⇒ Core → `LookPath("powershell")` ⇒ Desktop → Core default. `resolveRCPath` delegates, passing through the `platform domain.Platform` it already receives; `status` prints edition + path + provenance; `doctor` warns when `OtherExists`. | Writing both profiles; probing by running `pwsh -c $PROFILE`; re-deriving platform from `runtime.GOOS` inside `resolvePowerShellProfile` itself | Never both (resolved decision). `LookPath` is already on `Env`, so detection is a fake in tests and never spawns a process. The `platform` parameter (implemented; the signature above omitted it as shorthand) reuses `resolveRCPath`'s existing parameter and Phase 2's `$ALIASDECK_PLATFORM` test seam, so decision 9's Windows-only Documents/OneDrive branch and decision 10's non-Windows branch are both unit-testable on any host OS without needing a real Windows machine in CI. |
...
| 16 | `source.git.path` | OPTIONAL. Omitted ⇒ `aliases.yaml` at the checkout root. Present ⇒ joined against the checkout root and the result MUST resolve inside the checkout; a `..`-bearing path (or any path escaping the checkout via a symlink-free lexical join) is rejected before the file is read. | Requiring `path` always; resolving it against the process cwd | Mirrors `FileSource`'s existing path-omitted default (`Source.Path` empty ⇒ `AliasesFile(base)`), so a bare `source: {type: git, url: ...}` behaves the same way a bare `source: {type: file}` already does. The containment check is the same hostile-input posture as decision 12's hashed cache directory: a repository the user does not fully control must not be able to point `path` outside its own checkout. |
