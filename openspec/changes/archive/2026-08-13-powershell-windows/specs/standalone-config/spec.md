...
(Previously: detection covered only macOS/Linux and `$SHELL`; Windows had no supported platform value.)
...
(Previously: schema validation covered `file`/`server` source shapes only; `git` was unvalidated.)
